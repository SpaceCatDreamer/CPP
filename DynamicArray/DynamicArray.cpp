...
DynamicArray::DynamicArray(int length, int n) {
    if (length <= 0) {
        throw "Size of the array can't be less than 1";
    }
    this->length = length;
    dynamicArray = new int[length];
...
void DynamicArray::resize(int newSize)
{
   ...
   /* DynamicArray arr(newSize);
    if (newSize <= length) {
        for (int i = 0; i < newSize; i++) {
            arr.dynamicArray[i] = dynamicArray[i];
        }
    }
    else {
        for (int i = 0; i < length; i++) {
...
DynamicArray & DynamicArray::operator=(const DynamicArray &other)
{
    ...
    //memcpy(dynamicArray, other.dynamicArray, length);
...
bool operator>(DynamicArray &first, DynamicArray &second)
{
    for (int i = 0; i < std::fmin(first.getLength(), second.getLength()); i++) {
        if (first[i] <= second[i]) {
            return false;
        }
    }
